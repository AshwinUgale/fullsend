...
- Should trace scoring cover run *health*, not just trace *health*? Trace *fitness* ([ADR 0087](../ADRs/0087-eval-measurements-online-trace-scoring.md)'s `trace_fitness`) checks that a run's telemetry is well-formed — span tree, identity, model, usage, cost, and exit present and coherent — but not whether the run *behaved* correctly. A separate class of failure is structurally decidable straight from the trace, with no model in the loop: a tool that returned an error the run then ignored, an errored result reused by a later side-effecting call, no-progress loops, or a job that reports success despite an in-trace failure (e.g. [#6621](https://github.com/fullsend-ai/fullsend/issues/6621)). Two questions follow. First, does the eval-measure layer grow a second scorer for these run-*health* defects — the deterministic complement to the statistical evals in [testing-agents.md](testing-agents.md), catching what is provable from a single trace rather than what needs repeated runs — or does behavioral correctness belong with the review and retro agents instead? Second, there is a prerequisite either way: the structured-trace ideal described above records each tool call's result, but tool *results* are not on the trace yet (today's spans carry tool-call names and counts, not results), so the highest-value rules depend on first extending capture — itself a telemetry-contract decision. Prior discussion in [#7246](https://github.com/fullsend-ai/fullsend/issues/7246).
